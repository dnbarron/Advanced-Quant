...
Three level variable called _apply_, with levels "unlikely", "somewhat likely", and "very likely", coded 0, 1, and 2, respectively, that we will use as our outcome variable. Three explanatory variables: _pared_, dummy variable indicating whether at least one parent has a graduate degree; _public_, dummy variable indicating whether undergrad college is public or private, and _gpa_, student's grade point average. 
...
```{r, fig.width=7}
plot(Effect(c('pared', 'gpa'), o1), confint = FALSE, main = '',
     lines = list(multiline = TRUE, lwd = 3, lty = c(1, 2, 4)),
...
```{r, fig.width = 8}
plot(Effect(c('ses', 'write'), test), rug = FALSE, line = c(1, 2, 4),
     main = '', confint = FALSE,
     axes = list(x = list(rug = FALSE)),
     lines = list(lwd = 3, lty = c(1, 2, 4)))
```
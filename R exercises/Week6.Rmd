...
```{r bhps2}
selvar <- function(vn){
  require(reshape2) # You must install reshape2 package for this to work
  vars <- grep(vn, names(bhps), ignore.case=TRUE)
  if (length(vars) == 0) stop('No variables found')
  # First variable is pid, always include this
  op <- bhps[, c(1, vars)]
  # Turn variable into long format, using pid as id variable. 
  op.melt <- melt(op, id.vars='pid')
  # Create a wave variable from the first letter of the variable
  op.melt$wave <- substr(op.melt$variable, 1, 1)
  # Rename the value variable 
  names(op.melt)[3]  <- substr(op.melt$variable[1], 2, 100)
  # Output pid, selected variable and wave
  op.melt[, c(1,3,4)]
}

...
#influenceIndexPlot(weak1, labels = weak.inf$Country, id.n = 3, vars = "hat")
```
...
#residualPlots(weak1, type = "rstudent")
```
...
# influenceIndexPlot(weak1, vars = "Cook", labels = weak.inf$Country, id.n = 2)
```
...
#residualPlot(inter1, type = "rstudent")
```
...
plot(Effect('age', slid2), ci.style = 'none', rug = FALSE, main = '', xlab = 'Age', ylab = 'Log wages', ylim = c(2.1, 3.1))
...
Assume that $y_i$ is observed if and only if a second, unobserved latent variable, $z^*_i$ exceeds a particular threshold:
...
predicted probability (known as the Mills ratio) as an independent variable in the outcome equation.
...
```{r}
wom <- haven::read_dta("http://www.stata-press.com/data/r15/womenwk.dta")
